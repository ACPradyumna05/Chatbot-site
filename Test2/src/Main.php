...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isset($data['message']) || empty($_SESSION['UserID'])) {
        echo json_encode(['error' => 'Invalid input or missing session']);
        exit();
    }
    ...
    // Forward the request to app.py
    $url = 'http://127.0.0.1:5000/message';
    $postData = json_encode(['UserID' => $userId, 'message' => $userMessage]);
    ...
    if (curl_errno($ch)) {
        ...
        echo $response; // Forward the AI's response to the frontend
    }
...
<head>
    ...
    <script src="chatmsg.js" defer></script>
